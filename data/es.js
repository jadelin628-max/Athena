...
window.SUBJECTS.es = (function () {
  ...
  const DATA = [
    // ==================== 起步答疑 ====================
    F('bs01', 'start', '西语是入门成本最低的欧洲语言',
      R`为什么说西语「怎么写就怎么读」？`,
      R`西语拼读**近乎完全透明**：五个元音各一音永不变化、辅音与拼写一一对应、重音有明确规则（不确定的带重音符标出）。
推论：**一两周就能朗读任何西语文本**（哪怕不懂意思）——这是西语对初学者独一份的红利，法语英语都比不了。
你的记忆量因此全部集中在词汇与动词变位上——没有拼写税。`),
    F('bs02', 'start', '发音难点预览：只有一座山',
      R`西语发音难在哪？哪些是「假坑」？`,
      R`真山只有一座：**大舌颤音 rr**（perro 狗）——练习法（「的了」连音加速）phon 章有专卡，且不影响可懂度，可以慢慢磨。
假坑先排掉：**h 永远不发音**（hola 读「欧拉」）、b 与 v 同音、重音有规则可循——都是知道即会的送分点。
跟读工具：学习页 🔊 按钮（es-ES 口音），元音练「纯」是第一要务。`),
    F('bs03', 'start', '语法两大预告：ser/estar 与变位',
      R`西语语法的主战场在哪？`,
      R`两座主山：① **两个「是」**：ser 表本质、estar 表状态位置——英语/中文一个「是」，西语要先过思维转换关（noun 章专卡）；② **动词变位**——人称×时态变化，但规则性强（三组模板）+ 主语可省略。
应对：变位先吃透现在时三组模板与四大高频不规则（ser/estar/ir/tener），口语 80% 场景够用。
好消息预告：西语阴阳性比法语规律得多（-o 阳 -a 阴），按词尾猜准确率极高。`),
    F('bs04', 'start', '第一周怎么过',
      R`西语第一周的推荐节奏？`,
      R`① **拼读+元音开路**：phon 章全过 + 每天跟朗读按钮读——元音发「纯」是西语口音的命门（中文母语者注意 e/i 别吞糊）；② 初学者模式开「起步答疑 + 语音与拼读 + 名词与冠词」，每日 10 张；③ rr 每天一分钟练习即可，别给自己压力。
...
本卡组用法：语音卡配朗读按钮（es-ES）跟读；ser/estar 与 gustar 卡是思维重建重点。`),
  ...
  const META = {
    bs01: [4, '起步·西语'], bs02: [4, '起步·西语'], bs03: [4, '起步·西语'], bs04: [4, '起步·西语'], bs05: [4, '起步·西语'],
  ...
  return { BEGINNER: BEGINNER, id: 'es', name: '西班牙语', short: '西语', icon: '🌞', kind: 'qa', group: 'lang', CATS: CATS, DATA: DATA, META: META, REL: {}, PITFALL: PITFALL, MNEM: {}, SENTENCES: SENTENCES, ORDER: ['start', 'phon', 'noun', 'verb', 'sent', 'words', 'culture'] };
})();
