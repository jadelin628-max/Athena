...
window.SUBJECTS.fr = (function () {
  ...
  const DATA = [
    // ==================== 基础体系 ====================
    F('bs01', 'base', '字母表与拼读总表',
      R`26 个字母 + 组合规则的收束表？`,
      R`字母与英语同序；**核心组合规则收束**：eau/au/o → o；ai/ei → è；ou → u 音；oi → wa；gn → 捏音；ill 多数 → 伊音；ch → 英语 sh 音；h 永远哑音；词尾辅音多数不发音（c/r/f/l 常发音——记忆口诀 CREF）。
音符：é（闭口 e）、è/ê（开口 e）、ç（s 音）。
这张表是「见词能读」的全部家当——phon 章各卡的收束版。`),
    F('bs02', 'base', '三大变位族总表',
      R`-er / -ir / -re 三族的现在时骨架？`,
      R`**-er 族**（八成动词）：去 er 加 e/es/e/ons/ez/ent（parler → je parle）；**-ir 族**（finir 型）：去 ir 加 is/is/it/issons/issez/issent；**-re 族**：去 re 加 s/s/——/ons/ez/ent（prendre → je prends，单数第三人称无词尾）。
不规则主力另记：être/avoir/aller/faire/pouvoir/venir（verb 章逐卡）。
策略：先吃透 -er 族与六大不规则——日常对话覆盖九成。`),
    F('bs03', 'base', '宾语代词体系',
      R`le、la、les、lui、leur 放哪里？`,
      R`**直接宾语代词**：me/te/le/la/nous/vous/les（替代「被做的事」的对象——Je le vois 我看见他）；**间接宾语代词**：me/te/lui/nous/vous/leur（替代 à 某人——Je lui parle 我跟他说话）。
位置铁律：**放在动词前**（Je t'aime——我爱你）；否定环绕：Je ne le vois pas。
lui/leur 只给**人**（间接）；给物的 à 用 y——进阶再收。`),
    F('bs04', 'base', '日期与星期表达',
      R`星期几、几月几号怎么说？`,
      R`星期（首字母小写）：lundi 周一、mardi、mercredi、jeudi、vendredi、samedi、dimanche；月份：janvier、février、mars、avril、mai、juin、juillet、août、septembre、octobre、novembre、décembre。
句式：**le** lundi（每周一——定冠词表习惯）vs lundi（这个周一）；日期：le 3 mai（5 月 3 日——日序数只 1 用 premier）。
问日期：Quel jour sommes-nous aujourd'hui ?（今天星期几）。`),
    // ==================== 语音与拼读 ====================
...
这四个情态动词 + 原形 = 表达意愿的全部骨架——Je voudrais + Je peux 的组合就能礼貌走天下。`),
    F('se06', 'sent', '存在句：il y a',
      R`「有」怎么表达？`,
      R`**il y a** = 有/存在着（单复数同形）：Il y a un problème（有个问题）、Il y a des touristes（有一些游客）。
否定：Il n'y a pas de…（没有……——「否定句中冠词变 de」的规则在此同样生效）。
疑问：Est-ce qu'il y a…? / Y a-t-il…?（正式）——问路问店必用句。`),
    F('se07', 'sent', '天气句型：il fait',
      R`法语怎么聊天气？`,
      R`天气用无人称 **il fait…**：Il fait beau（天气好）、Il fait chaud/froid（热/冷）、Il fait 30 degrés（三十度）。
其他天气动词：Il pleut（下雨）、Il neige（下雪）——同样是无人称 il。
天气是法语 small talk 的万能开场——Il fait beau aujourd'hui ! 一句就能开启对话。`),
    F('se08', 'sent', '比较级：plus / moins / aussi',
      R`「更」「不如」「一样」怎么说？`,
      R`**plus…que**（比……更）、**moins…que**（不如……）、**aussi…que**（和……一样）：Paul est plus grand que moi（Paul 比我高）。
最高级：le/la plus + 形容词（le plus grand 最高）；bon/bien/mauvais 有不规则形式（meilleur/mieux/pire）。
注意口语里 plus 常不读尾音——比听出来的更重要是先会说。`),
    // ==================== 文化与场景 ====================
...
本卡组用法：语音卡配朗读按钮（fr-FR）跟读纠音，变位卡按 FSRS 滚动。`),
    ...
    F('fw31', 'words', 'le chat', R`le chat`,
      R`名词（阳）· 猫。搭配：un chat noir（黑猫）；狗：le chien；猫叫声 miaou。`),
    F('fw32', 'words', 'le chien', R`le chien`,
      R`名词（阳）· 狗。搭配：un petit chien（小狗）；阴性 la chienne。`),
    F('fw33', 'words', 'le livre', R`le livre`,
      R`名词（阳）· 书。搭配：lire un livre（读书）；注意 livre 同时是重量单位「斤」（un livre = 半公斤）。`),
    F('fw34', 'words', 'le café', R`le café`,
      R`名词（阳）· 咖啡；咖啡馆。点单：un café, s'il vous plaît（一杯咖啡，劳驾）；去咖啡馆喝一杯＝prendre un café。`),
    F('fw35', 'words', "l'argent", R`l'argent`,
      R`名词（阳）· 钱。搭配：Je n'ai pas d'argent（我没钱——否定 de 规则）；取款机：distributeur。`),
    F('fw36', 'words', 'le matin', R`le matin`,
      R`名词（阳）· 早上。搭配：le matin（在早上——定冠词表习惯）、tous les matins（每天早上）；晚上：le soir。`),
    F('fw37', 'words', 'le soir', R`le soir`,
      R`名词（阳）· 晚上。搭配：ce soir（今晚）、le soir（在晚上）；与 bonsoir（晚上好）同根。`),
    F('fw38', 'words', 'rire', R`rire`,
      R`动词 · 笑。搭配：rire de qqch（笑某事）；名词 le rire（笑声）；网络聊天「mdr」（mort de rire，笑死了）。`),
    F('fw39', 'words', 'courir', R`courir`,
      R`动词 · 跑。搭配：courir dans le parc（在公园跑步）；变位不规则（je cours / nous courons）。`),
    F('fw40', 'words', 'facile', R`facile`,
      R`形容词 · 容易（阴阳性同形）。搭配：C'est facile !（很简单！）；反义：difficile（难）。`),
    F('fw41', 'words', 'difficile', R`difficile`,
      R`形容词 · 难（阴阳性同形）。搭配：C'est difficile à dire（这很难说）；口语反说 pas facile（不容易）。`),
    F('fw42', 'words', 'fatigué', R`fatigué / fatiguée`,
      R`形容词 · 累。搭配：Je suis fatigué（我累了——男）/ fatiguée（女）；口头禅级：Je suis trop fatigué.（太累了）。`),
    F('fw43', 'words', 'heureux', R`heureux / heureuse`,
      R`形容词 · 幸福/高兴。搭配：Je suis heureux de…（我很高兴……）；祝福：Bonne année et bonne santé !（新年快乐身体健康）。`),
    F('fw44', 'words', 'ici', R`ici`,
      R`副词 · 这里。搭配：Viens ici !（过来！）；反义：là-bas（那里）；il y a 此处细节：ici il y a…（这里有……）。`),
    F('fw45', 'words', 'là-bas', R`là-bas`,
      R`副词 · 那里。搭配：Regarde là-bas !（看那边！）；与 ici 成对记：ici ↔ là-bas；单独的 là 也表「那里」。`),
  ];
  ...
  const HELP = [
    { id: 'bs01', title: '法语拼写复杂但拼读是规律的', body: R`有——而且相当规律。法语的「乱」在**写法**：同一读音多种拼写（eau/au/o 都读 o）；但**读法方向是死的**：认识字母组合规则后，见词能读。
推论：学法语的正确顺序是**先攻拼读规则**（phon 章一张表的事），此后阅读永远有读音抓手——别被拼写吓退，母语者小学也在学这个。` },
    { id: 'bs02', title: '发音难点预览：三座小山', body: R`三座小山，都能一周翻过：① **小舌音 r**（漱口的位置轻擦——每天含水练习法）；② **鼻化元音**（口型做元音、气流走鼻腔——四个音一套口诀）；③ **联诵**（前词尾辅音「复活」连读——规则明确，phon 章专卡）。
加一个「假坑」排除：词尾辅音大多不发音（Paris 的 s）、h 永远不发音——不是难点，是送分规律。
跟读工具：学习页 🔊 按钮（fr-FR 口音），每天跟三遍。` },
    { id: 'bs03', title: '语法两大预告：阴阳性与变位', body: R`两座主山：① **名词阴阳性**——每个名词自带性别（与意义无关），必须连冠词整体记（记 une table 不记 table）；② **动词变位**——人称×时态的词形变化体系。
应对：阴性阳性靠「从第一天就带冠词记忆」的习惯，变位靠「先吃透高频四大」（être/avoir/aller/faire），规则动词一族一套模板。
verb 章会按这个顺序展开——预告在此，到时别慌。` },
    { id: 'bs04', title: '第一周怎么过', body: R`① **拼读规则先吃透**：phon 章每天 2-3 张 + 跟朗读按钮读词例——这是法语入门最值的一次投资（此后见词能读）；② 初学者模式开「起步答疑 + 语音与拼读 + 名词与冠词」，每日 10 张；③ 每天大声跟读五分钟——法语靠节奏组整块模仿，不开口学不会。
...
总对策：法语是「前期规则投资大、后期阅读回报高」的语言——phon 章 investment 一次，终身受用。` }
  ...
  return { BEGINNER: BEGINNER, id: 'fr', name: '法语', short: '法语', icon: '🗼', kind: 'qa', group: 'lang', CATS: CATS, DATA: DATA, META: META, HELP: HELP, REL: {}, PITFALL: PITFALL, MNEM: {}, SENTENCES: SENTENCES, ORDER: ['phon', 'base', 'noun', 'verb', 'sent', 'words', 'culture'] };
})();
