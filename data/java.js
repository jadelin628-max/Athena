...
arch: '架构与规则',
  ...
  const DATA = [
    // ==================== 架构与规则 ====================
    F('ar01', 'arch', 'JVM 运行时数据区',
      R`JVM 的内存是怎么划分的？`,
      R`五大区域：① **堆**——对象与数组实例的驻地（垃圾回收的主战场，线程共享）；② **虚拟机栈**——每个线程私有，方法调用的栈帧（局部变量表）；③ **方法区/元空间**——类信息、静态变量、运行时常量池；④ **程序计数器**——当前线程执行到哪条字节码；⑤ **本地方法栈**——native 方法。
要点：对象在堆、引用（局部变量）在栈——NullPointerException 的「空引用」就是栈上的引用没指向堆里的对象。`),
    F('ar02', 'arch', '类加载：加载、链接与初始化',
      R`类从 .class 文件到可用经历了什么？`,
      R`三步：① **加载**——读 .class 字节码进方法区，生成 Class 对象；② **链接**——验证（合法性）→ 准备（静态变量分配默认值）→ 解析（符号引用转直接引用）；③ **初始化**——执行静态变量赋值与 static 块（按代码顺序，只一次）。
触发初始化的时机：new、访问静态成员、反射等——「主动使用」才初始化，父类先于子类。
理解它就懂了：为什么 static 块只跑一次、为什么循环依赖要小心。`),
    F('ar03', 'arch', '访问修饰符：四档可见性',
      R`public/protected/默认/private 各自可见范围？`,
      R`四档从宽到严：**public**（所有地方）→ **protected**（同包 + 子类）→ **默认（包私有，不写修饰符）**（同包）→ **private**（仅本类）。
规律：修饰**类**只有 public 与默认两种；修饰成员四档全可用。
实践：字段一律 private（封装起点），方法按需逐步放宽——从最严开始，比先公开再收紧容易得多。`),
    F('ar04', 'arch', 'Java 只有值传递',
      R`方法传参到底是传值还是传引用？`,
      R`铁律：**Java 只有值传递**。传基本类型 → 拷贝值本身；传对象 → 拷贝「引用的值」（两个引用指向同一个堆对象）。
推论：方法内**改对象的内容**（list.add、obj.field=x）对调用方可见；**给参数重新赋值**（param = new ...）只改了拷贝的引用，调用方无感。
经典面试题：swap 两个对象引用为什么失败——因为交换的只是两份引用拷贝。`),
    F('ar05', 'arch', '标准库地图：常用包导览',
      R`JDK 的常用包有哪些？`,
      R`**java.lang**（自动导入：String/Math/基本包装类）；**java.util**（集合/Scanner/Date——最常用）；**java.io 与 java.nio**（文件与流）；**java.time**（现代日期时间——lib 章专讲）；**java.net**（网络）；**java.util.concurrent**（并发工具——conc 章主场）。
查阅习惯：先看 java.lang 与 java.util——八成需求 JDK 自带，别急着自己造轮子。`),
    // ==================== 基础与 JVM ====================
...
高层替代：BlockingQueue 的 take 与 put 自动完成等待与通知。`),
  ...
  const HELP = [
    { id: 'bs01', title: 'Java 适合做什么', body: R`**强类型编译型语言**，跑在 JVM 上——企业后端（银行、电商）、Android（早期主力）、大数据（Hadoop 生态）的支柱语言，就业面极广。
哲学：「一次编写，到处运行」+ 严格的结构（一切皆对象、强类型）——啰嗦但稳，团队协作友好。
对初学者：语法比 Python 严格（要声明类型），换来的是大型项目里的可靠性与工具支持。` },
    { id: 'bs02', title: '安装 JDK 与版本选择', body: R`装 **JDK**（开发工具包，含编译器）——直接装 **LTS 版本**（17 或 21），别装只含运行环境的 JRE。
验证：终端敲 java -version 与 javac -version 都出版本号（java 跑程序、javac 编译——两个都要有）。
包含关系：**JDK ⊃ JRE ⊃ JVM**——JDK 是全套工具箱，JVM 是最终执行字节码的引擎。` },
    { id: 'bs03', title: '第一个程序：类与主方法', body: R`Java 万物皆类：文件名必须与 public 类同名（HelloWorld.java），程序入口是 main 方法（固定签名：public static void main）。
两步运行：**javac HelloWorld.java**（编译出 .class 字节码）→ **java HelloWorld**（JVM 执行）。
初见觉得啰嗦（打印一句话要套三层壳）——这正是 Java 结构化风格的缩影，习惯它。` },
    { id: 'bs04', title: 'JVM 与字节码：跨平台的原理', body: R`javac 把源码编译成 **.class 字节码**（不是机器码）——字节码是给 JVM 看的中间指令。每个平台装对应版本的 JVM，它把字节码翻译成本机指令。
所以：程序不用改，各平台装好 JVM 就能跑——JVM 替你扛住了平台差异。
直觉：Python 解释器逐句执行源码，Java 先编译成统一中间码再由 JVM 执行——工程化与性能兼得的关键设计。` },
    { id: 'bs05', title: '怎么读异常堆栈', body: R`报错是**异常堆栈**：**从上往下读**——第一行是异常类型与消息（如 NullPointerException），下面每个 at 行是一层调用位置。
...
最常见的三个：NullPointerException（空指针）、ArrayIndexOutOfBoundsException（下标越界）、ClassCastException（类型转换错）——先混个脸熟。` },
    { id: 'bs06', title: '包与 import：代码的组织', body: R`**package** 声明类所在的「文件夹」（命名空间）：域名倒写是惯例（com.company.project）——防止全世界的类重名。
**import** 引入别的包的类（import java.util.List）——不写就要用全限定名。IDE 会自动管理 import，但要理解它做了什么。
一个文件只能有一个 public 类，且类名必须与文件名一致——Java 的物理组织纪律。` },
    { id: 'bs07', title: '工具链总览：IDE 与构建工具', body: R`**IDE**：IntelliJ IDEA（社区版免费，事实标准）或 VS Code 加扩展——Java 手写编译运行太啰嗦，IDE 一键完成。
**构建工具**：Maven / Gradle——管理依赖（第三方库）与构建流程，pom.xml/build.gradle 是它们的配置。初学先知道名字，引第一个第三方库时再学。
**JDK 自带三件套**：javac（编译）、java（运行）、javadoc（生成文档）。` },
    { id: 'bs08', title: 'Java 学习路径建议', body: R`① 起步章 + 基础章打底（类型、运算、控制流）；② **面向对象章是 Java 的灵魂**——类/继承/接口/多态必须吃透，这一章值得双倍时间；③ 集合框架（List/Map/Set）是日常代码的主力容器；④ 异常与泛型让代码健壮；⑤ 并发是进阶，入门期不急。
节奏参考：每天 10 张卡 + 每周一个小程序（学生管理、简易记账）。
提醒：IDE 的自动补全很好用，但**语法要手写到肌肉记忆**——面试与调试都靠它。` }
  ];

  const BEGINNER = ['basic', 'oop'];
  return { BEGINNER: BEGINNER, id: 'java', name: 'Java', short: 'Java', icon: '☕', kind: 'qa', group: 'skill', CATS: CATS, DATA: DATA, META: META, HELP: HELP, REL: {}, PITFALL: PITFALL, MNEM: {}, ORDER: ['arch', 'basic', 'oop', 'coll', 'exgen', 'lib', 'conc'] };
})();
